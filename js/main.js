$(document).ready( function() {
    ...
    function addAnimationWhenScroll(element) {
        setTimeout(function() {
            let rect = element.getBoundingClientRect();
            let heightScreen = window.innerHeight
            if (!(rect.top > heightScreen || rect.bottom < 0)) {
    ...
    function startAnimationScroll () {
        document.querySelectorAll('.title-field').forEach(function (element) {
            addAnimationWhenScroll(element)
        })
    ...
    carBtns.forEach(function(item, idx) {
        item.addEventListener('click', function (){
            ...
            document.querySelector('.described').innerHTML = `
                <div class="feature">
                    <h3>${vehicleInformation[idx].price}</h3>
                    <p>Starting MSRP</p>
                </div>
                <div class="feature">
                    <h3>${vehicleInformation[idx].MPG}</h3>
                    <p>MPG city/hwy</p>
                </div>
                <div class="feature"> 
                    <h3>${vehicleInformation[idx].maxAvailableHp}</h3>
                    <p>Max Available hp</p>
                </div>
                <div class="feature">
                    <h3>${vehicleInformation[idx].maxCargoVolume}</h3>
